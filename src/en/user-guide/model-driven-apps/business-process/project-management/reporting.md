---
author: David Hudec
---

# Reporting

Reporting is the other half of project management beside direct task oversight.

## Reporting IDs

To keep reporting flexible, each task and milestone is connected to a **Reporting ID**. Reporting lists then hold ordered lists of reporting IDs, or they are used in reports directly.

This means semantically similar tasks or milestones can still be reported together even when they:

- exist in different projects
- come from different template versions
- appear at different hierarchy levels
- sit in different reporting sequences for different audiences

## Why it matters

The same reporting identity can appear:

- as one step in a finance report
- as another step in a production report
- or not appear in a vendor report at all

This keeps reporting flexible without forcing the delivery structure itself to match one reporting hierarchy.
